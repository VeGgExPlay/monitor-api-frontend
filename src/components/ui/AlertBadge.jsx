...
export default function AlertBadge() {
  ...
  useEffect(() => { load(); }, []);
  useEffect(() => { const interval = setInterval(load, 30000); return () => clearInterval(interval); }, []);

  if (loading) return null;

  return (
    <Link to="/alerts" className="relative p-2 rounded-md text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
